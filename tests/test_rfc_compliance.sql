...
-- SKIP=OMIT is the default, so RSCALE should remain NULL
INSERT INTO rfc_test_results (test_name, status)
...
VALUES ('RSCALE=GREGORIAN without SKIP',
    assert_occurrences_equal(
        ...
        ARRAY[
            '2025-01-31 10:00:00'::TIMESTAMP,  -- Jan 31 ✓
            '2025-03-31 10:00:00'::TIMESTAMP,  -- Feb 31 skipped (default OMIT)
            '2025-05-31 10:00:00'::TIMESTAMP,  -- Apr 31 skipped (default OMIT)
            '2025-07-31 10:00:00'::TIMESTAMP   -- Jun 31 skipped (default OMIT)
        ],
        (SELECT array_agg(occurrence) FROM "all"(
...
BEGIN
    ...
    result := (SELECT array_agg(occurrence) FROM "all"(
...
VALUES ('Complex RRULE with SKIP (auto-add RSCALE)',
    assert_occurrences_equal(
        ...
        ARRAY[
            '2025-01-31 14:30:00'::TIMESTAMP,  -- Jan 31 ✓
            '2025-02-28 14:30:00'::TIMESTAMP,  -- Feb 31 → 28 (BACKWARD)
            '2025-03-31 14:30:00'::TIMESTAMP   -- Mar 31 ✓
        ],
        (SELECT array_agg(occurrence) FROM "all"(
...
-- Test 13: RSCALE + SKIP + BYDAY interaction
INSERT INTO rfc_test_results (test_name, status)
VALUES ('RSCALE + SKIP + BYDAY',
    assert_occurrences_equal(
        'RSCALE + SKIP + BYDAY filter',
        ARRAY[
            '2025-01-31 10:00:00'::TIMESTAMP,  -- Jan 31 = Friday ✓
            '2025-02-28 10:00:00'::TIMESTAMP   -- Feb 31 → 28 (BACKWARD), Feb 28 = Friday ✓
        ],
        (SELECT array_agg(occurrence) FROM "all"(
            'FREQ=MONTHLY;BYMONTHDAY=31;RSCALE=GREGORIAN;SKIP=BACKWARD;BYDAY=FR;COUNT=2',
            '2025-01-01 10:00:00'::TIMESTAMP
        ) AS occurrence
    )
);

-- Test 14: RSCALE + SKIP with leap year
INSERT INTO rfc_test_results (test_name, status)
VALUES ('RSCALE + SKIP with leap year',
    assert_occurrences_equal(
        'RSCALE + SKIP leap year handling',
        ARRAY[
            '2024-02-29 10:00:00'::TIMESTAMP,  -- Leap year: Feb 30 → 29 (BACKWARD)
            '2025-02-28 10:00:00'::TIMESTAMP,  -- Normal year: Feb 30 → 28 (BACKWARD)
            '2026-02-28 10:00:00'::TIMESTAMP   -- Normal year: Feb 30 → 28 (BACKWARD)
        ],
        (SELECT array_agg(occurrence) FROM "all"(
...
-- Test 15: RSCALE + SKIP with multiple BYMONTHDAY values (deduplication)
INSERT INTO rfc_test_results (test_name, status)
