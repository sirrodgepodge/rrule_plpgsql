...
\echo '==================================================================='
\echo ''
\echo 'TIMESTAMP API (timezone via TZID in RRULE, DST-aware):'
...
\echo ''
\echo 'TIMESTAMPTZ API (timezone via explicit parameter, DST-aware):'
\echo '  - rrule.all(rrule, dtstart_tz, timezone) -> SETOF TIMESTAMPTZ'
\echo '  - rrule.between(rrule, dtstart_tz, start_tz, end_tz, timezone) -> SETOF TIMESTAMPTZ'
\echo '  - rrule.after(rrule, dtstart_tz, after_tz, count, timezone) -> SETOF TIMESTAMPTZ'
\echo '  - rrule.before(rrule, dtstart_tz, before_tz, count, timezone) -> SETOF TIMESTAMPTZ'
\echo ''
\echo 'Convenience methods:'
\echo '  - rrule.next(rrule, dtstart) -> TIMESTAMP [from NOW]'
\echo '  - rrule.most_recent(rrule, dtstart) -> TIMESTAMP [before NOW]'
\echo ''
\echo 'Advanced:'
\echo '  - rrule.overlaps(dtstart, dtend, rrule, mindate, maxdate) -> BOOLEAN'
\echo ''
\echo 'Features:'
